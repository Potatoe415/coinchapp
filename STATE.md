...
Current_Goal: Bouilla end-of-game screen now shows winner + full round scores table.
Last_Action: Extracted `BouillaScoreTable` from `BouillaScoreboard.tsx` (shared sub-component); overlay reuses it. In `BouillaRoundOverlay.tsx`, the finished state's 2x2 score grid replaced with `BouillaScoreTable`; panel widened to max-w-md and scrollable (max-h-[90vh] overflow-y-auto). TypeScript clean.
Next_Actions:
...
- 2026-07-17 Added an online-only "are you still there?" idle-turn timer + permanent bot takeover for both games. See DECISIONS.
