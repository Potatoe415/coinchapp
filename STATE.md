...
Current_Goal: Manually validate the 3 latest online/UX fixes (own-card instant play, pre-selection, and reduced peer-move latency), then resume la Bouilla's English UI validation across local/online/ad-hoc modes.
    10|Last_Action: Fixed a real online-with-humans latency report ("peer's move takes ~2s to appear on host's screen"): `getView`/`playCard`/`submitBotMove` were each stacking 3-5 *sequential* Supabase round trips (`getUserId()` then `loadGame()`'s own two selects, plus a presence write and a backup `game_events` insert). Now `getUserId()`/`loadGame()` run concurrently, `loadGame`'s two selects run concurrently, and the two non-authoritative writes (presence heartbeat, backup realtime-tick row) are deferred via Next's `after()` instead of blocking the response. With user confirmation, also switched `getUserId()` itself from `supabase.auth.getUser()` to `getClaims()` (still verifies the JWT, but locally when the project's signing keys allow it - see DECISIONS for the security tradeoff accepted). Also earlier: (1) "card takes 1-2s to play online" for Bouilla - instant optimistic hand/trick update, fixed via a new shared `lib/client/useOptimisticPlay.ts` hook also adopted by Coinche's `GameTable`; (2) missing "pre-select a card while waiting for your turn" in Bouilla, added to that same hook (`preSelectedId`/`tapCard`) plus a shared `components/HandCardSlot.tsx` for the lift+ring visual. See DECISIONS for all. Type-check, lint, Vitest (108), and production build all pass.
Next_Actions:
- Manually play online with a real second device + bots: confirm the peer's card now appears noticeably faster on the host's screen than before (some real network latency will remain - only avoidable round trips were removed).
