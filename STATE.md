...
Last_Action: Root-caused and fixed the intermittent 3-15s per-bot-move delay in online Bouilla. The debug overlay (built last session) showed the "handoff" column (gap before the next bot even starts deciding) spiking to 12-13s while decide/submit/refetch stayed normal (~2s total) - pointing at pure client-side render/effect churn, not network/compute. Root cause: `useBotRunner`'s effect keyed on the `gv` view object, which gets a brand-new reference on *every* refetch, including redundant ones the bot's own move triggers (its `game_events` insert echoes back via postgres_changes to the same client) and the 15s safety poll. Each such redundant refetch re-ran the effect and (via cleanup) cancelled the next bot's in-flight decide, discarding it before it could submit; a fresh attempt then had to restart from scratch, each retry burning a full `thinkMs` cycle (≥800ms) - explains why the delay got *worse* after the thinking-time pacing was added (bigger collision window). Fixed by gating re-entrancy on the turn number (`activeTurnRef`) instead of on `gv` identity, so redundant same-turn refetches are ignored instead of restarting the attempt (`useBotRunner.ts`). Tests (122)/build/lint green; deployed.
Next_Actions:
- Ask the user to retest online Bouilla (solo host + 3 bots) with the debug overlay still on and confirm the handoff column now stays low (near the previous ~300-500ms baseline) instead of spiking to 12-13s.
