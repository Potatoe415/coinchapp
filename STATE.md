...
Last_Action: Fixed the user-reported Bouilla bot errors ("they play their aces when they should not take the trick; they never build coupes sèches"). Root cause of the aces: `sweepAliveFor` is true after a single trick and exactly one seat wins trick 1 of every round, so from trick 2 all four bots were in "win the trick" mode (one chasing a Capot, three racing to break it). New `sweepProgress` (`rounds.ts`) now gates both modes behind `SWEEP_CHASE_PROGRESS = 0.5` (sweep at least half secured). Void-building added on both sides: `chooseLead` prefers our own shortest suit (control cards excluded - they would win the trick), and `byMostDanger` now orders discards as danger -> control card -> shortest suit -> highest rank. 200-deal simulation: aces/kings in tricks 1-3 fell from 661/2400 to 75/2400 (tricks round), empty suits after 6 tricks rose 3.69 -> 4.40 per deal. 130 tests green (was 122), typecheck clean, lint unchanged (pre-existing `useMatchStats.ts` error only). Logged in `docs/DECISIONS.md`.
Next_Actions:
